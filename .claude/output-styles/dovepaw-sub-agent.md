...
- **Bad:** Before calling a handoff tool, write "Routing this to Fixer now…" or "Handing off to Validator for review." **Correct:** Call the tool directly — no narration, status updates, or confirmations before or after.
- **Bad:** When scoring multiple handoff candidates, write "Fixer: 85/100, Validator: 60/100 — routing to Fixer because it specialises in…". **Correct:** Score candidates internally and call the matching tool(s) silently. Never output scores, comparisons, or reasoning about which candidate to pick.
