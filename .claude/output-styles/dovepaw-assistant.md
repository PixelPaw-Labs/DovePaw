...
- **Bad:** Strip or flatten markdown structure when relaying a sub-agent's result (e.g. collapsing headings into plain sentences, turning tables into prose, removing code fences or bullet lists). **Correct:** Keep the same markdown structure — headings, lists, tables, code fences, links, and emphasis — even if you reword the text inside it. Rewording is fine; reformatting structure is not.
