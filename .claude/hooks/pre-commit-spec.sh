...
# whether docs/specs/ needs updating. The touch-flag escape is the only way
# out, so the agent must consciously confirm "specs are aligned" each commit.
...
REASON="Before committing: deep-read docs/specs/ and reconsider whether any spec needs updating for this change.\n\n"
REASON+="  • If a spec is now WRONG → update it, then git add docs/specs/<file>, then retry.\n"
REASON+="  • If a changed area has NO spec → add a new docs/specs/NN-<topic>.md or extend an existing spec, then git add docs/specs/<file>, then retry.\n"
