...
async function writeMemoryScript(
  ...
  const portFile = `${process.env.HOME}/.dovepaw/.openviking-port.json`;
  const script = `#!/usr/bin/env bash
# Generated by DovePaw — do not edit
AGENT_ID="${contextId}"
PORT=$(python3 -c "import json; print(json.load(open('${portFile}'))['port'])" 2>/dev/null)
[ -z "$PORT" ] && exit 0
...
case "$CMD" in
  read)
    PAYLOAD=$(python3 -c "import json,sys; print(json.dumps({'query':sys.argv[1],'target_uri':'viking://agent/memories','limit':10}))" "\${*:-memories}")
  ...
  save)
    ...
    curl -s --max-time 5 -X POST "\${BASE}/api/v1/sessions/\${SID}/messages" \\
      ...
      -H "Content-Type: application/json" \\
      -d @- >/dev/null 2>&1
    curl -s --max-time 5 -X POST "\${BASE}/api/v1/sessions/\${SID}/commit" \\
      -H "X-OpenViking-Agent: \${AGENT_ID}" \\
      -H "Content-Type: application/json" \\
      -d '{}' >/dev/null 2>&1
    ;;
