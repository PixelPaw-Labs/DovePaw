...
import { messageText, type MessageSegment, type ToolCall } from "@/components/hooks/use-messages";
import { AnimatedMessage } from "./animated-message";
import { CopyAction } from "./copy-action";
import { ThinkingDots } from "./thinking-dots";
import { EditDiffList, ToolCallChain } from "./tool-call-badge";
...
function groupSegments(segments: MessageSegment[]): SegmentGroup[] {
  ...
  while (i < segments.length) {
    ...
    if (seg.type === "tool_call") {
      ...
      while (i < segments.length) {
        const s = segments[i];
        if (s.type !== "tool_call") break;
        tools.push(s.tool);
        i++;
      }
      groups.push({ type: "tool_call_group", tools });
    } else {
...
export function ChatMessageItem({ msg }: { msg: ChatMessage }) {
  ...
  const groups = groupSegments(msg.segments);
          ...
          {groups.map((group, i) =>
            group.type === "text" ? (
              ...
              <ToolCallChain
                ...
                isActive={!!msg.isLoading && i === groups.length - 1}
              />
            ),
          )}
